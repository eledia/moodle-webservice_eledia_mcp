...
$string['setting_allow_token_in_query_help'] = 'When enabled, the MCP endpoint accepts the token through the <code>?wstoken=</code> query parameter. Disabled by default because tokens in URLs leak into web server logs, browser history, and HTTP referer headers. Clients should use the <code>Authorization: Bearer</code> header.';
...
$string['setting_allowed_origins_help'] = 'One origin per line (e.g. <code>https://app.example.com</code>). Use <code>*</code> to allow any origin (not recommended). When empty, only same-origin requests are accepted. The MCP server validates the <code>Origin</code> header on all requests and returns HTTP 403 for any origin not on this list.';
...
$string['setting_emergency_disable_help'] = 'When enabled, every request to the MCP endpoint returns HTTP 503 Service Unavailable. Use this as a temporary kill switch during incident response.';
...
$string['setting_enforce_mcp_service_help'] = 'When enabled (default), the MCP endpoint only accepts tokens that belong to a configured MCP external service. This makes the MCP service list the access boundary for the endpoint, not just for token issuance. Disable only if you must present tokens minted for other web services.';
...
$string['setting_expose_raw_functions_help'] = 'When enabled, every external function assigned to the authenticated service is exposed as an MCP tool, in addition to the curated AI-native tools. Disabling this restricts the surface to the AI-native tool set only, which is recommended for production AI agents.';
$string['setting_expose_raw_functions_warning'] = 'Security warning: enabling raw Web Service functions greatly expands the tool surface exposed to AI clients. Use this only for controlled administrator testing, not as the default for production agents.';
...
$string['setting_max_request_size_help'] = 'Requests with a body larger than this value are rejected with HTTP 413. Default 1 MiB.';
...
$string['setting_rate_limit_per_hour_help'] = 'Maximum number of requests per hour for a single token. Default 600.';
...
$string['setting_rate_limit_per_minute_help'] = 'Maximum number of requests per minute for a single token. Default 60.';
...
$string['setting_services_help'] = 'The external services that may issue MCP tokens. Only services selected here can be chosen in the self-service token UI or targeted through the internal token API. Create the services under <em>Site administration → Server → Web services → External services</em> first, then enable them here.';
...
$string['setting_token_retention_days_help'] = 'How many days to keep the audit record of a revoked MCP token before the scheduled cleanup task deletes it. Connector-provisioned tokens are re-minted regularly, so their revoked records can accumulate. Set to 0 to keep every revoked token record indefinitely.';
...
$string['setting_tools_page_size_help'] = 'Maximum number of tools returned per <code>tools/list</code> response. Larger sets are paginated through the <code>nextCursor</code> field.';

...
final class tool_provider_test extends externallib_advanced_testcase {
    ...
    public function test_get_tools(): void {
        ...
        // Locate the raw function tool (skipping AI-native tools registered above).
        $rawtool = null;
        foreach ($tools as $t) {
            if ($t['name'] === 'core_webservice_get_site_info') {
                $rawtool = $t;
                break;
            }
        }
        $this->assertNotNull($rawtool, 'Raw Moodle Web Service tool should be present in the catalogue.');
        $this->assertArrayHasKey('description', $rawtool);
        $this->assertArrayHasKey('inputSchema', $rawtool);
        $this->assertArrayHasKey('outputSchema', $rawtool);
        $this->assertArrayHasKey('annotations', $rawtool);
        $this->assertArrayHasKey('readOnlyHint', $rawtool['annotations']);
        $this->assertTrue($rawtool['annotations']['readOnlyHint']);
        ...
        // Legacy clients still get the wrapper.
        $legacytools = tool_provider::get_tools(
            $token->token,
            \webservice_elediamcp\local\protocol::LEGACY
        );
        $rawlegacy = null;
        foreach ($legacytools as $t) {
            if ($t['name'] === 'core_webservice_get_site_info') {
                $rawlegacy = $t;
                break;
            }
        }
        $this->assertNotNull($rawlegacy);
